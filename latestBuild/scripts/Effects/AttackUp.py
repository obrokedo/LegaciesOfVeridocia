...
class AttackUp(BattleEffect):
    ...
    def effectStarted(self, attacker, target):    
        ...
        if effectLevel == 1:
            self.effectValue = int(target.getCurrentAttack() * 0.25)
        elif effectLevel == 2:
            self.effectValue = int(target.getCurrentAttack() * 0.30)
        elif effectLevel == 3:
            self.effectValue = int(target.getCurrentAttack() * 0.35)
        elif effectLevel == 4:
            self.effectValue = int(target.getCurrentAttack() * 0.40)

        target.setCurrentAttack(target.getCurrentAttack() + self.effectValue)
         
        return None;
    
    # This is the text indicating what effect has been applied
    def effectStartedText(self, attacker, target):
        return target.getName() + " attack was raised."

...
class Burst(SpellDefinition):    
    # This is where you set up all of the parameters for each of the spells    
    def __init__(self):
        self.setName("Burst")       
        self.setCosts(jarray.array([4, 10], 'i'))
        self.setDamage(jarray.array([-10, -20], 'i'))
        self.setRange(jarray.array([Range.ONE_ONLY, Range.TWO_AND_LESS], Range))
        self.setArea(jarray.array([2, 2], 'i'))            
    ...
    def getBattleText(self, target, damage, mpDamage, attackerHPDamage, attackerMPDamage):
        return "burst " + target.getName() + ", dealing " + `damage * -1` + " damage! "

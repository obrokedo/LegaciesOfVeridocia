...
{	
			...
			CombatSprite targetSprite, StateInfo stateInfo, int baseConfidence, int cost, Point attackPoint, int distance)
	{
		...
		if (spell.getDamage() != null && spell.getDamage().length >= i && spell.getDamage()[0] < 0)
			handleDamagingSpell(spell, i, tileWidth, tileHeight, currentSprite, targetSprite, stateInfo, baseConfidence, cost, distance);
		else
			handleHealingSpell(spell, i, tileWidth, tileHeight, currentSprite, targetSprite, stateInfo, baseConfidence, cost, attackPoint);
	}
